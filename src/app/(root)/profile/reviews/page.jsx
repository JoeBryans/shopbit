...
const page = async () => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/review`,
    {
      method: "GET",
      headers: {
  ...
  return (
    <div className='w-full min-h-[100vh]  flex flex-col items-center mt-10'>
      <h1 className="text-2xl font-bold mb-5 -m-t-5 ">Reviews</h1>
              ...
              return <div key={index}>
                <div className="flex md:w-[500px] w-full gap-3 shadow-lg px-3 mt-5 border-b-2 h-44 py-1 items-center rounded-lg mx-auto">
                  <div className=" w-36 h-28 flex items-center justify-center rounded-lg">
                    <Image
                      src={reviews?.product?.images[0]?.url}
                      alt="image"
                      width={100}
                      height={100}
                      priority
                  ...
                  <div className="flex flex-col gap-1 w-72  ">
                    <div className="">
                      <span className="">{reviews?.product?.category?.name}</span>
                      <span className="line-clamp-1">{reviews?.product?.name}</span>
                      <Currency price={reviews?.product?.price} />
